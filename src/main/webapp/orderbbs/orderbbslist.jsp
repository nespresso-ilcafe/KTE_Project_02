...
<center>
    ...
    <table width=1000 border=0 style="font-size:13px;text-align:center">
    	<tr><td colspan=4 align=left style="font-size:18px;font-weight: bold">ORDER NOTICE</td></tr>
        ...
        <c:forEach var="list" items="${m }">
            <tr>
                ...
                <td align=left>
                	<c:choose>
                		<c:when test="${sessionScope.id == list.id || sessionScope.level == 10 }">
                			<a href="orderbbsview?uid=${list.uid }">${list.subject }</a>
                		</c:when>
    ...
    <table width=1000 border=0 style="text-align: center">
        <tr>
            ...
            <c:choose>
                ...
                <c:otherwise><td width=100 align=right><a href="orderbbswrite">[게시물등록]</a></td></c:otherwise>
            </c:choose>
        </tr>
    </table>
</center>
